...
const IndividualRegister = () => {
  ...
  useEffect(() => {
    const checkFormValidity = () => {
      const { name, phoneNumber, password, confirmPassword, residency } = formData;
      const isFormValid =
        name &&
        phoneNumber &&
        password &&
        ...
        <div className="w-full px-8">
          <form onSubmit={handleSubmit}>
            ...
            <div className="flex items-center justify-center">
              <button
                type="submit"
                className={`w-1/5 p-3 text-lg bg-blue-900 text-white rounded transition-transform ${!isFormValid ? 'opacity-50 cursor-not-allowed' : 'hover:scale-105'}`}
